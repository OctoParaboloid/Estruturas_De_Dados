...
int **CriaVetorFilas_inteiros(int n, char **entrada)
{
    int **v = (int **)calloc(n, sizeof(int *));
    char s[2] = " ";
    ...
    {
        char *token;
        token = strtok(entrada[i], s);
        int tam_k = atoi(token);

        v[i] = (int *)calloc(tam_k+1, sizeof(int));
                                   //primeiro valor indica quantos clientes

        int j = 0;
        while (j < tam_k+1 && token != NULL)
        {
            v[i][j] = atoi(token);
            token = strtok(NULL, s);
            j++;
...
char **cria_entrada(int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        entrada[i] = (char *)calloc(1000, sizeof(char));  //no maximo 100 numeros
    }
    return(entrada);
}
...
int *vetor_intercalado(int **VF, int n_filas) //a ideia é intercalar iterativamente
{
    ...
    int *intercalado = (int *)calloc(tam_intercalado+1, sizeof(int));
    ...
    {
        intercalado_antigo = intercalado;
...
int main()
{
    ...
    scanf("%d\n", &n_filas);
        if (n_filas > 0){
            ...
            for (int i = 0; i < n_filas; i++)
            {
                fgets(entrada[i], 1000, stdin);        //preencho cada fila com os clientes
            }
            //printa_entrada(entrada, n_filas);
                //--------FIM_LEITURA---------
            int **VF = CriaVetorFilas_inteiros(n_filas, entrada);
            ...
            if (n_filas > 1) free(intercalado_final);
