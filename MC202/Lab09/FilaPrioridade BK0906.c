...
{
    int maior = i;
    if ((2*i + 1 < h->qtde) && (h->vetor_clientes[2*i + 1] > h->vetor_clientes[maior]) )
        maior = 2*i + 1;
    if ((2*i + 2 < h->qtde) && (h->vetor_clientes[2*i + 2] > h->vetor_clientes[maior]) )
        maior = 2*i + 2;
...
void insere_max_heap(Heap *h, cliente *novo_cliente)
{
    ...
    {
        h->qtde++;
        if (h->vetor_clientes[h->qtde-1] != NULL) free(h->vetor_clientes[h->qtde-1]);
...
cliente *remove_max_heap(Heap *h)
{
    if (h->qtde > 0)
    {
    h->qtde--;
...
cliente *remove_min_heap(Heap *h)
{
    if (h->qtde > 0)
    {
    h->qtde--;
